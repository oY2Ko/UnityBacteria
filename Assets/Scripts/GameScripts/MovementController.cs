...
public class MovementController : MonoBehaviour
{
    ...
    private void OnCollisionEnter2D(Collision2D foodCollision) // Перенести в отдельный скрипт 
    {
        ...
        if (foodCollision.gameObject.tag == FoodTag)
        {
            ...
            {
                PhotonNetwork.Destroy(foodCollision.gameObject);
            }
            else
            {
                CustomData = characteristics["Attack"].ToString() + " " + foodCollision.gameObject.name;
                PhotonNetwork.RaiseEvent(1, CustomData, Options, SendOptions);
